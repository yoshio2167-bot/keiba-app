...
st.set_page_config(
    page_title="競馬予想AIシミュレーター（2Dコース＆自動化版）",
    layout="wide",
    initial_sidebar_state="collapsed",
)

st.title("競馬予想AIシミュレーター ＆ 2Dレース展開ビジュアライザー")
...
with tab1:
  st.header("モンテカルロ・シミュレーション ＆ 2Dレース展開図")
  st.write(
      "出馬表CSVを貼り付けて予算を設定すると、AIがレース展開を自動判定し、コース上の位置取り（2Dアニメーション風）と3連複配分を算出します。"
  )
  ...
  if pasted_data.strip():
    try:
      ...
      if df_input is not None and not df_input.empty:
        ...
        df_input["speed_val"] = df_input["スピード指数"].apply(
            lambda x: extract_num(x, 0.0)
        )
        ...
        st.dataframe(
            df_input[available_preview], use_container_width=True, height=180
        )

    except Exception as e:
      st.info("CSVデータを貼り付けるとここにプレビューが表示されます。")

  if st.button("🚀 2D展開シミュレーション ＆ 予想を実行", type="primary"):
    if df_input is not None and not df_input.empty:
      with st.spinner("AIがレース展開を自動判定・コース展開を構築中..."):
        ...
        def calc_enhanced_score(row):
          ...
          try:
            ...
            first_num = (
                int(rec.split("-")[0])
                if "-" in rec and rec.split("-")[0].isdigit()
                else 5
            )
            if first_num <= 3:
        ...
        # ---------------------------------------------------------
        # 🏟️ 2Dコース展開図（トラックビジュアライザー）の描画
        # ---------------------------------------------------------
        st.subheader("🏟️ 2Dコース位置取りシミュレーション（直線・4コーナー付近）")
        fig, ax = plt.subplots(figsize=(10, 4))
        ax.set_facecolor("#1e3f20")  # 芝のグリーン
        fig.patch.set_facecolor("#0e1117")

        # トラックの枠線を描画
        ax.plot([0, 100, 100, 0, 0], [0, 0, 50, 50, 0], color="white", lw=2)
        ax.axvline(x=80, color="yellow", linestyle="--", label="ゴール前直線")
        ax.text(
            81,
            25,
            "GOAL",
        ...
        # 各馬をスコア（位置取り）に応じてプロット
        # 上位馬ほど前（右側・ゴール寄り）、下位は後方
        max_score = df_ranked["_win_num"].max() if len(df_ranked) > 0 else 1.0
        if max_score == 0:
          max_score = 1.0

        for i, row in df_ranked.iterrows():
          # x座標: スコアが高いほどゴール(80付近)に近い
          score_val = row["_win_num"]
          x_pos = 10 + 70 * (score_val / max_score)
          # y座標: 1頭ずつバラす
          y_pos = 5 + (i * 45.0 / max(1, len(df_ranked)))

          umaban = row["馬番"]
          name = row["馬名"]

          color = "red" if i == 0 else ("orange" if i == 1 else "cyan")
          marker = "o" if i < 3 else "s"
          size = 120 if i < 3 else 60
        ...
        kaisai_title = (
            str(df_display["開催地"].iloc[0])
            if not df_display["開催地"].empty
            else "阪神"
        )
        r_num_title = (
            str(df_ranked["レース番号"].iloc[0])
            if not df_ranked["レース番号"].empty
            else "1R"
        )
        ...
        three_renpuku_focus = (
            ...
            f" （合計投資: {bet_amount_per_point * num_bets_3ren}円）"
        )
        ...
        csv_download_data = df_export_final.to_csv(index=False).encode(
            "utf-8-sig"
        )
        st.download_button(
        ...
        if roi_val_num >= threshold_roi:
          st.success(
              f"🔥 **【勝負レース推奨】（期待回収率: {top1['AI期待回収率_str']}"
              f" ＞ 設定基準 {threshold_roi}%）**\n\n{strict_buy_focus}\n\n{three_renpuku_focus}\n\n※期待値が高いため、ワイドおよび3連複軸2頭流しで高回収を狙えます。"
          )
        else:
          st.warning(
              f"⚠️ **【見送り推奨 / パス】（期待回収率: {top1['AI期待回収率_str']}"
...
with tab2:
  ...
  st.write(
      "保存したシミュレーション結果（CSV）をアップロードし、実際の1〜3着馬を選択すると、的中状況を自動判定します。"
  )

  uploaded_sim_file = st.file_uploader(
      "1. シミュレーション結果CSVをアップロード", type=["csv"]
  )
  ...
  if uploaded_sim_file is not None:
    ...
    date_val = (
        str(df_saved["日付"].iloc[0])
        if "日付" in df_saved.columns and not df_saved["日付"].empty
        else "2026-09-20"
    )
    kaisai_val = (
        str(df_saved["開催地"].iloc[0])
        if "開催地" in df_saved.columns and not df_saved["開催地"].empty
        else "阪神"
    )
    r_num_val = (
        str(df_saved["レース番号"].iloc[0])
        if "レース番号" in df_saved.columns and not df_saved["レース番号"].empty
        else "1R"
    )
    dist_val = (
        str(df_saved["距離・馬場"].iloc[0])
        if "距離・馬場" in df_saved.columns
        and not df_saved["距離・馬場"].empty
        else "芝1200m(良)"
    )
    cond_val = (
        str(df_saved["レース条件"].iloc[0])
        if "レース条件" in df_saved.columns
        and not df_saved["レース条件"].empty
        else "オープン"
    )

    top1_row = df_saved.iloc[0]
    win_rate_val = str(
        top1_row.get("シミュ勝率", top1_row.get("シミュ勝率_str", "0%"))
    )
    place_rate_val = str(
        top1_row.get("シミュ複勝率", top1_row.get("シミュ複勝率_str", "0%"))
    )
    roi_val = str(
        top1_row.get("AI期待回収率", top1_row.get("AI期待回収率_str", "0%"))
    )
    ...
    t1 = (
        f"◎{ai_top3_list[0]['num']}番 {ai_top3_list[0]['name']}"
        if len(ai_top3_list) > 0
        else ""
    )
    t2 = (
        f"〇{ai_top3_list[1]['num']}番 {ai_top3_list[1]['name']}"
        if len(ai_top3_list) > 1
        else ""
    )
    t3 = (
        f"▲{ai_top3_list[2]['num']}番 {ai_top3_list[2]['name']}"
        if len(ai_top3_list) > 2
        else ""
    )
    ...
    with col1:
      actual_1st = st.selectbox(
          "🥇 実際の1着馬", options=["選択してください"] + horse_options, index=0
      )
    with col2:
      actual_2nd = st.selectbox(
          "🥈 実際の2着馬", options=["選択してください"] + horse_options, index=0
      )
    with col3:
      actual_3rd = st.selectbox(
          "🥉 実際の3着馬", options=["選択してください"] + horse_options, index=0
      )

    if st.button("🔍 検証結果を自動判定する"):
      if (
          actual_1st == "選択してください"
          or actual_2nd == "選択してください"
          or actual_3rd == "選択してください"
      ):
        ...
        if hit_count >= 2:
          auto_memo = (
              f"的中（上位3頭から {hit_count}頭が馬券内絡み・ワイド的中圏内）"
          )
          badge_type = "success"
        elif hit_count == 1:
...
with tab3:
  ...
  st.write(
      "カンマ区切りの出馬表データをここに貼り付けると、15列の正しいフォーマットに完璧に整えます。"
  )

  raw_txt = st.text_area(
      "ここにカンマ区切りの出馬表データを貼り付け",
      placeholder=(
          "2026-09-20,阪神,1R,芝1200m(良),オープン,1,ルクスルーラー..."
      ),
      height=150,
  )
  ...
  if st.button("✨ 完璧なCSVに変換する"):
    if raw_txt:
      ...
      if parsed_rows:
        ...
        st.success(
            "変換が完了しました！下のボックスをコピーしてTab1に貼り付けてください。"
        )
